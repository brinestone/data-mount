...
function Upsert-TranslationsFromBase {
  param(
  ...
  foreach ($baseUnit in $BaseUnits) {
    ...
    if ($null -eq $localeUnit) {
      ...
      elseif ($localeSource.InnerText -ne $baseSource) {
        ...
        if ($null -ne $target) { $target.InnerText = "" }
      }
    }
  ...
  $baseIds = @{}
  foreach ($bu in $BaseUnits) { $baseIds[$bu.GetAttribute("id")] = $true }
...
foreach ($locale in $locales) {
  ...
  Upsert-TranslationsFromBase -LocalePath $filename -Locale $locale -BaseUnits $baseUnits
